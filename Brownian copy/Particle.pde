class PartType{
  float radius;
    ...
    Particle Gen(PVector dPos,PVector dVel,float dAngle, float dAngVel, float dAngAcc){
      return new Particle(type,dPos,dVel,radius,mass,charge,
      dAngle, dAngVel, dAngAcc,momInert,
      bondLength,bondAngle,bondK,bondAngK); 
    }
    Particle Gen(PVector dPos,PVector dVel,float dAngle, float dAngVel, float dAngAcc, Particle dP){
      return new Particle(type,dPos,dVel,radius,mass,charge,
      dAngle, dAngVel, dAngAcc,momInert,
...
class Particle implements Comparable{
    ...
    Particle(int dType, PVector dPos,PVector dVel,float dRad,float dMass,float dCharge,
    ...
    Particle(int dType,PVector dPos,PVector dVel,float dRad,float dMass,float dCharge,float dAngle, float dAngVel, float dAngAcc,float dMomInert,float dBondLength,float dBondAngle,float dBondK,float dBondAngK,Particle dP){
      type=dType;
      pos=dPos; vel= dVel; radius= dRad; mass= dMass; charge= dCharge; angle= dAngle; angVel=dAngVel;angAcc=dAngAcc; momInert=dMomInert; bondLength=dBondLength; bondAngle=dBondAngle; bondK=dBondK; bondAngK=dBondAngK;
      bondAngK2=bondAngK;
      bonded=true;
