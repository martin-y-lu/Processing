...
class ChemSys{
 ...
 int SPEED=0;
 PVector GRAV=new PVector(0,0.0004);
 float ENERGY=0;
 PVector Size;
 
 ChemSys(ArrayList<PartType> dTypes,String[] dSymbols, float[][] dHBondRadius,float[][] dHBondForce,float[][] dCBondLength, float[][] dCBondK, float[][] dCBondEnergy, float[][] dCBondExtra,float[][] dCActivation, PVector dSize){
  Types= dTypes;
  Symbols=dSymbols;
 ...
 ChemSys(ArrayList<PartType> dTypes, String[] dSymbols, PVector dSize){
    Types= dTypes;
    Symbols=dSymbols;
    //Particles= dParticles;
    Particles= new ArrayList<Particle>( );
    Bonds= new ArrayList<Bond>();
    NUMTYPES=dTypes.size();
    hBondRadius= new float[NUMTYPES][NUMTYPES];
 ...
 void SetHydrogenBond(int AInd,int BInd, float BondRadius, float BondForce){
   hBondRadius[AInd][BInd]=BondRadius; 
   hBondRadius[BInd][AInd]=BondRadius; 
   hBondForce[AInd][BInd]=BondForce; 
   hBondForce[BInd][AInd]=BondForce; 
 }
 void SetCovalentBond(int AInd,int BInd, float BondLength, float BondK,float BondEnergy, float BondExtra, float BondActivation){
 ...
 PVector EField(PVector Pos){
  ...
  Bond AddBondBetween(Particle A,int AInd,Particle B,int BInd){
    ...
    Bond BOND=new Bond(cBondLength[A.type][B.type],cBondK[A.type][B.type],energy,A,AInd,B,BInd);
    A.Sites[AInd]=BOND;
    B.Sites[BInd]=BOND;
    Bonds.add(BOND);
    return BOND;
...
class PartType{
  float radius;
    ...
    Particle Gen(ChemSys dSys,PVector dPos,PVector dVel,float dAngle, float dAngVel, float dAngAcc, Particle dB,int AInd,int BInd){
      Particle P= new Particle(dSys,type,dPos,dVel,radius,mass,charge,
      dAngle, dAngVel, dAngAcc,momInert,
      bondAngles,bondAngKs);
