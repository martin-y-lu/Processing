 ...
 PVector EField(PVector Pos){ // E field at point
  ...
  float PotentialBondEnergy( Particle A, int AInd, Particle B, int BInd){ // Potential bond energy IF there were to be a covalent bond between Particle A, and Particle B in the specified bonding sites
    ...
    float pot=-bondEnergy;// Bond energy is negativer potential
     ...
     //pot+= cDipole[A.type][B.type]*(A.eenergy[A.electron]-B.eenergy);
     if(cDipole[A.type][B.type]>0){
       pot+= cDipole[A.type][B.type]*(A.differencialE(-1)+B.differencialE(1));
     }else{
       pot+= -cDipole[A.type][B.type]*(A.differencialE(1)+B.differencialE(-1)); 
     }
     return pot;
  ...
  void CheckCollision(Particle A, Particle B){ // CHecks all collision dynamics between pair of particles, determines if new bond should be formed, and creates it
    if(A.Collides(B)){// If physically colliding
      ...
      float eDiff=0;
      do{
        //Testing posititive etransfer ( A->B)
        
       println("add");
       float pos= A.differencialE(A.electrons-etransfer,-1)+B.differencialE(B.electrons+etransfer,1);
       //Testing negative etransfer
       
       println("remove");
       float neg= +A.differencialE(A.electrons-etransfer,1)+B.differencialE(B.electrons+etransfer,-1);
       eDiff= max(pos,neg);
       //println("EDIFF:"+eDiff);
       println("A"+A.type+" B"+B.type);
       println((A.electrons-etransfer)+"  "+(B.electrons+etransfer));
       //println("add:"+pos+"  "+A.differencialE(A.electrons-etransfer,-1)+"  "+B.differencialE(B.electrons+etransfer,1));
       //println("remove"+neg+ "  "+A.differencialE(A.electrons-etransfer,1)+"  "+B.differencialE(B.electrons+etransfer,-1));
       println("etransfer:"+etransfer);
       if(eDiff>0.1){
        if(pos>neg){
          if((A.electrons+2*A.numBonds()-etransfer>0)&&(B.electrons+2*B.numBonds()+etransfer<B.emax)){
          etransfer++; 
          }else{
           break;
          }
        }else{
          if((A.electrons+2*A.numBonds()-etransfer<B.emax)&&(B.electrons+2*B.numBonds()+etransfer>0)){
            etransfer--; 
          }else{
           break;
          }
      ...
      //  etransfer= min(A.electrons,B.emax-B.electrons-2*B.numBonds());
      //  maxEnergy=etransfer*(B.eenergy-A.eenergy);
      //}if(A.eenergy>B.eenergy){// B e's to A
      //  etransfer= -min(B.electrons,A.emax-A.electrons-2*A.numBonds());
      //  maxEnergy=etransfer*(B.eenergy-A.eenergy);
      //}
