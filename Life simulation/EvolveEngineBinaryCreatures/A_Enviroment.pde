class Enviroment{
  ...
  String[] SpeciesFitnessInfo= new String[NUMCREATURES];
  ...
  void Trim(float frac){
    ...
    //float variance=150*NUMCREATURES;
    ...
    float variance=100*NUMCREATURES;
    for(int i=0;i<Live.length;i++){
      Live[i]= i<cutPoint;
      float gauss=exp(-pow((float(i)-cutPoint),2.0)/(2.0*variance))/2.0;
      if(random(1)<gauss){
  ...
  void Reproduce(){
    ...
    while(TestList.size()>NumOrganisms){
        TestList.remove(NumOrganisms-1);
    }
    //TestList.clear(); //TEST TEST TEST TEST TESTS TETE
    //for(int i=0; i<OList.size();i++){
    //  ShiftToTest(i);
    //}    
  }
