class Smart{
  ...
  void MutateSmart(float MuteAmount){
    for(int i=0;i<Weights.length;i++){
      Weights[i]+=MuteAmount*randomGaussian()*0.03;
      if(Weights[i]<-20){
        Weights[i]=-20;
      }
      if(Weights[i]>20){
    ...
    Bias+=MuteAmount*randomGaussian()*0.04;
    if(Bias<-100){
       Bias=-100; 
    }if(Bias>100){
       Bias=100; 
...
class Eye extends Point{
  ...
  Eye(PVector dPos,PVector dVel,float dMass,float  dFriction, ArrayList<PVector> dRayDirs, float dWeight[],float dBias){
     ...
     for(int i=0;i<dRayDirs.size();i++){
       EyeRays.add(new Ray(dPos, dRayDirs.get(i).copy()));
     }
  }
  Point ClonePoint(){
  ...
  void DrawP(Camera Cam,PGraphics Canvas){// Draws Eye.
    ...
    for(int i=0; i<EyeRays.size();i++){
      ...
      Canvas.strokeWeight(1.3*Cam.Zoom);
      for(float j=0;j<=1;j+=1.0/100){
         PVector Point=PVlerp(Origin,End,j);
         float state=Activation(PVDivide(Disp,EyeRay.Dir).x*Weights[i]);
         if(state>0){
            Canvas.stroke(state*200,0,255,Weights[i]*15+30);
          }else{
            Canvas.stroke(0,-state*200,255,Weights[i]*15+30);
          }
...
class Muscle extends Smart{
  ...
  void DrawM(Camera Cam ,PGraphics Canvas){// Draws Muscle.
    ...
    Canvas.stroke(0,0,0,(Force/(0.5)+0.1)*255);
