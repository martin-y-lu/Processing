...
class LatticePoint implements Comparable{//Represents a point in the order 5 square lattice
  ...
  HE_Mesh genMesh(PMatrix3D transform){
    HE_Mesh mesh= new HE_Mesh();
    if(coords.type==LatticeType.ORIGIN) return mesh;
    
    mesh.add(generateMeshLine(transform,BRANCHLENGTH));
    //drawLine3D(transform,BRANCHLENGTH);
    if(coords.type==LatticeType.BRANCH2){
      mesh.add(generateMeshLine(transform,BRANCHLENGTH,TWO_PI/5));
    }
    mesh.add(generateMeshSphere(transform,14));
    return mesh;
  }
  void update(){
     for(LatticeObject attachedObject:attachedObjects){
