...
HE_Mesh genMesh(){
  ...
  for(LatticeWalker w:lSystem.latticeWalkers){
        PMatrix3D matrix=w.renderPosition.getMatrix();
        HE_Mesh thisMesh = w.basePoint.genMesh(matrix);
        mesh.add(thisMesh);
  }
  HEM_HyperProject modifier=new HEM_HyperProject();
  modifier.setUnit(new WB_Vector(0,0,80));
  modifier.setScale(ScaleMesh);
